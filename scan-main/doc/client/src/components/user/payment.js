...
const Space = () => {
  return(
    <div>SCAN CLINIC :{clinic} <br></br>PLACE : {place}<br></br> TYPE OF SCAN: {scan}<br></br> DURATION:{time}</div>
  )
}
// const St = () => {
//   return(
...
const Rup = () => {
  return(
    <div> TOTAL Rs. {ruppess} <br></br> PAYMENT TYPE CARD</div>
  )
}
              ...
              <Box sx={{ mb: 2 }}>
                <div>
                  <Button
                    ...
                    sx={{ mt: 1, mr: 1 }}
                  >
                    {index === steps.length - 1 ? <div><a href="http://localhost:9000/" style={{backgroundColor:"transparent",textDecoration:"none",color:"white"}}>Pay</a></div> : 'Continue'}
                  </Button>
                  <Button
                    disabled={index === 0}
                    onClick={handleBack}
